...
CloudKit sync lets standalone-mode state (subscriptions, reading state, reactions, reading list, and archive preferences) stay in sync across all of a user's devices automatically via iCloud. Companion mode does not use CloudKit — it syncs through the web server instead.

### How it works

SwiftData's `.automatic` CloudKit mode handles everything for the synced state store:
- Schema mirroring: synced SwiftData models are automatically mapped to CloudKit record types
...
The infrastructure is built into `ModelContainerSetup.swift`, and `AppConfiguration` reads the container settings from your xcconfig.

NebularNews now uses two SwiftData configurations:
- a CloudKit-synced store for standalone account state such as feed subscriptions, synced article state, and synced archive preferences
- a local-only store for content cache and operational/read-model data such as feeds, articles, jobs, Today snapshots, summaries, images, and personalization tables
...
4. **Verify the entitlements file** was generated by Xcode with:
   ...
   <array>
       <string>iCloud.com.nebularnews</string>
   </array>
   ```
...
- All CloudKit-synced `@Model` properties must be optional or have default values (ours already do) — CloudKit requires this for schema compatibility
