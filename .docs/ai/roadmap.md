...
- [ ] `[minor]` **Polish widget visual design**: Current widgets (StatsWidget, TopArticleWidget, ReadingQueueWidget, NewsBriefWidget) work functionally but look plain. Tighten typography scale, use brand accent color, tune padding for lock-screen families, add graceful empty/stale states, consider a hero gradient for large family. Files: `NebularNewsWidgets/*.swift`.
