...
- `[~]` **Standardize logging**: `_shared/logger.ts` created with `log`/`logError` helpers, but **not yet adopted** — 20+ `console.log`/`console.error` calls across all functions still use raw logging. Needs a follow-up pass to import and use the logger.
