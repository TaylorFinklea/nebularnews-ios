...
actor ArticleFallbackImageService {
    ...
    // The curated Unsplash preset catalog remains the fallback image source until
    // a separate live search integration is designed with caching and attribution.

    init(modelContainer: ModelContainer) {
        self.articleRepo = LocalArticleRepository(modelContainer: modelContainer)
    }

    func ensureFallbackImage(articleID: String) async -> String? {
    ...
    private func selectPreset(
        ...
        if let themeKey = await selectThemeWithFoundationModels(for: snapshot),
           let preset = unsplashFallbackPreset(named: themeKey) {
            return ImagePresetSelection(preset: preset, provider: AIGenerationProvider.foundationModels.rawValue)
        }
